    ...
    render({ meta, added /* removed, changed */ }) {
      ...
      function renderAnnouncements(list) {
        ...
        <ul style="margin:0 0 0 -12px;padding:0 0 0 24px;font-size:13px;color:#111827;line-height:1.6;">
          ${list.map(item => {
          const title = item.title || "-";
          const href = item.href || null; // watcher parseNewData'dan geliyor
      ...
      const metaName = meta?.name || "MASAK";
      const metaUri = meta?.uri || "https://masak.hmb.gov.tr/duyuru/";
      const metaTrDate = meta?.trDate || new Date().toLocaleDateString("tr-TR");
...
<html>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>KKDF Mevzuatı Güncelleme</title>
  </head>
